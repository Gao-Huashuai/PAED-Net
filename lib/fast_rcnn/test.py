...
def im_detect(sess, net, im, boxes=None, save_feature=False, feature_path=None):
    ...
    cls_score, cls_prob, bbox_pred, rois = \
        sess.run([net.get_output('cls_score'), net.get_output('cls_prob'), net.get_output('bbox_pred'),net.get_output('rois')],\
                 feed_dict=feed_dict)
    if save_feature:
        conv5_3 = sess.run(net.get_output('conv1_2'), feed_dict=feed_dict)
        print('Saves the conv5_3 feature...')
        np.save(feature_path, conv5_3)
